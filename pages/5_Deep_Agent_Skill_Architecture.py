...
st.set_page_config(page_title="Deep Agent Skill Architecture", page_icon=None, layout="centered",
                   initial_sidebar_state="expanded")

apply_deep_agent_css()

st.markdown('<div class="page-title">Deep Agent Skill Architecture</div>', unsafe_allow_html=True)
st.markdown('<div class="page-caption">Plans its work, operates over a file workspace, and loads Skills on demand.</div>', unsafe_allow_html=True)
...
with col_chat:
    ...
    if len(st.session_state.da_messages) == 1:
        starter_clicked = render_starters([
            "Create a sample budget spreadsheet",
            "Analyze the data I uploaded",
            "Write a short summary as a Word doc",
...
user_input = st.chat_input("Ask the deep agent to build or analyze something...") or starter_clicked
    ...
    with col_chat:
        ...
        with st.chat_message("assistant"):
            ...
            try:
                ...
                # Don't render response_text here — st.rerun() below redraws the
                # whole conversation through the single history loop further up,
                # which is the only place a message is ever drawn. Rendering it
                # both here and there is what caused the final answer to briefly
                # appear twice (once normal, once faded) right as the next turn
                # started.
                st.session_state.da_messages.append(
